function go_next() {
  if (document.formm.okon1[0].checked == true) {
    document.formm.action = "ShopController?command=join_form";
...
function go_save() {
	if (document.formm.id.value == "") {
		...
		document.formm.action = "ShopController?command=join";
		document.formm.submit();
	}
}
...
function idcheck() {
	...
	var url = "ShopController?command=id_check_form&id="
...
function post_zip() {
	var url = "ShopController?command=find_zip_num";
	window.open(url, "_blank_1",
